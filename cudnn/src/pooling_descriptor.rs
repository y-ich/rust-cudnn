...
impl PoolingDescriptor {
    ...
    pub fn new(mode: cudnnPoolingMode_t, window: &[i32], padding: &[i32], stride: &[i32]) -> Result<PoolingDescriptor, Error> {
        ...
        try!(API::set_pooling_descriptor(generic_pooling_desc, mode, cudnnNanPropagation_t::CUDNN_NOT_PROPAGATE_NAN, window.len() as i32, window.as_ptr(), padding.as_ptr(), stride.as_ptr()));
